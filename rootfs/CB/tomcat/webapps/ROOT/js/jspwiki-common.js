...
String.prototype.repeat = String.prototype.repeat || function(size) {
...
String.prototype.deCamelize = String.prototype.deCamelize || function() {
	return this.replace(/([a-z])([A-Z])/g, '$1 $2');
}
// parse color : prefix with # if amtched with 3 or 6 hex codes
var REparseColor = new RegExp("^[0-9a-fA-F]+");
String.prototype.parseColor = String.prototype.parseColor || function() {
	var s = this;
	if (((s.length == 6) || (s.length == 3)) && REparseColor.test(s))
		s = "#" + s;
	return (s);
}

// replace xml chars by &entities;
String.prototype.escapeXML = String.prototype.escapeXML || function() {
...
var jspwikiExt = {
	...
	getElementsByClassName: function(className, matchFirst) {
	...
	setCookie: function(name, value, expires, path, domain, secure) {
		...
		/*
		 * Store the cookies agains the basepath of wiki so that different
		 * URLformats are supported properly !
		 */
		if (!path)
			path = Wiki.getBasePath();
	...
	getCookie : function(name) {
		...
		return (reMatchCookie.test(document.cookie) ? decodeURIComponent(RegExp.$1)
				: null);
	}
...
/**
 * Find the wiki content on the page which should be initialized with the
 * jspwiki-common scripts
 *
 * @param context if present the wiki content will be searched in the context of this jquery object
 * @return The jQuery collection of HTML elements which which contain rendered
 *         wiki text
 */
function findWikiContent(context) {
	// historically the wiki content was marked with the "pagecontent" id
	var pagecontent = context ? context.find("#pagecontent") : $("#pagecontent");
	// now the wiki-content is marked by the "wikiContent" css class
	var wikiContent = null;
	try {
		wikiContent = context ? context.find(".wikiContent") : $(".wikiContent");
	} catch (ignored) {
		// alert("error:" + ignored);
		// jquery thwrows an NPE here if there is no .wikiContent on the page
		// and this is called during page-load event
		// upgrading to jquery 1.7.2 does not help
	}
	if (pagecontent.length != 0 && wikiContent) {
		wikiContent = wikiContent.filter(function() {
			// remove those wikiContent elements which are inside the
			// #pagecontent already to avoid double initialization
			var childOfPageContent = $.contains(pagecontent[0], this);
			return !childOfPageContent;
		});
	}
	var all = pagecontent.add(wikiContent);
	return all;
}
...
Wiki.getBaseURL = function() {
	return this.BaseURL;
} // not yet used
Wiki.getBasePath = function() {
	return this.BasePath;
}
Wiki.getPageName = function() {
...
Wiki.showImage = function(attachment, attDELIM, maxWidth, maxHeight) {
	...
	// not clean: should actually be read from the attachImg size - but dont
	// know yet how xbrowser
	this.maxWidth = maxWidth;
	this.maxHeight = maxHeight;

	if (!this.reImageTypes.test(attachArr[0])) {
		attachImg.innerHTML = i18n.message("ajax.jspwiki.no.image.selected");
		return;
	}
	...
	if (this.pic.complete)
		return Wiki.showLoadedImage();

	this.countdown = 30;
	setTimeout("Wiki.showLoadedImage()", 200);
	attachImg.innerHTML = i18n.message("ajax.jspwiki.loading.image");
}
...
Wiki.showLoadedImage = function() {
	...
	if (this.pic.complete) {
		...
		attachImg.innerHTML = "<img src='" + this.pic.src + "' width='"
				+ parseInt(w) + "' height='" + parseInt(h)
	...
	if (this.countdown <= 0) {
		attachImg.innerHTML = i18n.message("ajax.jspwiki.loading.image.failed");
		return;
	}
...
Wiki.onPageLoad = function() {
	...
	var c = jspwikiExt.getCookie("JSPWikiUserPrefs");
	if (c == null)
		c = "";
	...
	var u = jspwikiExt.getCookie("JSPWikiUserProfile");
 ...
 *
 * issue a document.write statement with the link to the browser specific
 * stylesheet should always be execute from direct javascript during page-load
 */
...
TabbedSection.onPageLoad = function() {
	var t = jspwikiExt.getElementsByClassName("tabbedSection");

	if (!t)
		return;

	if ($(t).closest('.editor-wrapper').hasClass('wysiwyg')) {
		return;
	}
	for ( var i = 0; i < t.length; i++) {
		if (!t[i].hasChildNodes)
			continue; // take next section
		if ($(t[i]).hasClass("tabs")) {
		...
		for ( var n = t[i].firstChild; n; n = n.nextSibling) {
			if (!this.reMatchTabs.test(n.className))
				continue; // not a tab: take next element

			if ((n.id == null) || (n.id == "")) {
				var uniquePostfix = (i > 0) ? "-" + i : ""; // first occurrence does not need a postfix
				n.id = n.className + uniquePostfix;
			}
...
TabbedSection.onclick = function(tabId) {
	...
	for ( var n = section.firstChild; n; n = n.nextSibling) {
		...
		if (m && m.className == "activetab") {
			if (n.id == target.id)
				break; // stop - is already activetab
			...
			if (edittab && edittab.style.display == "") {
				...
				if (window.onTabChange
						&& typeof window.onTabChange === 'function') {
...
/**
 * * 120 SearchBox * Remember 10 most recent search topics * Uses a cookie to
 * store to 10 most recent search topics * * Extensions for quick link to View
 * Page, Edit Page, Find as is. * (based on idea of Ron Howard - Nov 05)
 */
var SearchBox = new Object();

SearchBox.submit = function(queryValue) {
	for ( var i = 0; i < this.recentSearches.length; i++) {
		if (this.recentSearches[i] == queryValue)
			return;
	}
...
SearchBox.onPageLoad = function() {
	...
	if (!this.recentSearchesDIV)
		return;

	this.recentSearches = new ExtArray();
	var c = jspwikiExt.getCookie("JSPWikiSearchBox");
	if (c)
		this.recentSearches = c.split(Wiki.DELIM);
	...
	var s = i18n.message("ajax.jspwiki.recent.searches") + ":";
	var t = [];
	...
	s += "<br /><div onclick='SearchBox.clearRecentSearches()'>"
			+ i18n.message("ajax.jspwiki.clear.recent.searches") + "</div>";
	this.recentSearchesDIV.innerHTML = s;
}

SearchBox.doSearch = function(searchDiv) {
	this.searchForm.query.value = searchDiv.innerHTML; // nodeValue seems not
														// to work
	this.searchForm.submit();
}

SearchBox.clearRecentSearches = function() {
	jspwikiExt.setCookie("JSPWikiSearchBox", "");
	this.recentSearches = new ExtArray();
...
ZebraTable.onPageLoad = function() {
	var z = jspwikiExt.getElementsByClassName(this.REclassName);
	if (!z)
		return;

	for ( var i = 0; i < z.length; i++) {
		var rows = z[i].getElementsByTagName("TR");
		if (!rows)
			continue;
...
/**
 * * 290 Highlight Word * * Inspired by
 * http://www.kryogenix.org/code/browser/searchhi/ * Modified 20021006 to fix
 * query string parsing and add case insensitivity * Modified 20030227 by
 * sgala@hisitech.com to skip words * with "-" and cut %2B (+) preceding pages *
...
HighlightWord.ClassNameMatch = "<span class='" + HighlightWord.ClassName
		+ "' >$1</span>";
HighlightWord.ReQuery = new RegExp("(?:\\?|&)(?:q|query)=([^&]*)", "g");

HighlightWord.onPageLoad = function() {
	if (!this.ReQuery.test(document.referrer))
		return;
	...
	findWikiContent().each(function() {
		self.walkDomTree(this);
	});
...
HighlightWord.walkDomTree = function(node) {
	...
	if (node.nodeType != 3)
		return;
	if (node.parentNode.className == this.ClassName)
		return;
	var s = node.nodeValue;
	s = s.escapeXML(); /*
						 * bugfix - nodeValue apparently unescapes the xml
						 * entities ?!
						 */
...
Sortable.onPageLoad = function(context) {
	Sortable.TitleSort = i18n.message("ajax.jspwiki.sortable.title.sort");
	Sortable.TitleSortAscending = i18n
			.message("ajax.jspwiki.sortable.title.sort.ascending.order");
	Sortable.TitleSortDescending = i18n
			.message("ajax.jspwiki.sortable.title.sort.descending.order");

	var self = this;
	findWikiContent(context).each(function() {
		var sortables = getElementsByClassName(this, Sortable.ClassName);
		if (!sortables)
			return;
		for ( var i = 0; i < sortables.length; i++) {
			...
			for ( var j = 0; j < table.rows[0].cells.length; j++) {
				...
				c.title = self.TitleSort;
				c.className += " " + self.ClassSort;
			}
		}
	});
}

Sortable.REclassName = new RegExp(
		'(?:^| )(sort|sortAscending|sortDescending)(?: |$)');
Sortable.sort = function(thNode) {
	var table = getAncestorByTagName(thNode, "table");
	if (!table)
		return;
	if (table.tBodies[0])
		table = table.tBodies[0]; // bugfix
	if (table.rows.length < 2)
		return;
	...
	for ( var i = 0; i < table.rows[0].cells.length; i++) {
		...
		if (thNode == c) {
			colidx = i;
			if (Sortable.REclassName.test(c.className))
				thNodeClassName = RegExp.$1;
		} else {
			c.className = c.className.replace(Sortable.REclassName, "") + " "
					+ this.ClassSort;
	...
	for ( var i = 1; i < table.rows.length; i++) {
		...
		if (num)
			num = !isNaN(parseFloat(val));
		if (date)
			date = !isNaN(Date.parse(val));
		if (ip4) {
			if (!val.match(ip4_regex)) {
				ip4 = false;
			}
		}
	}
	...
	if (thNodeClassName == this.ClassSort) // first time sort of column
											// table.sortCol == colidx )
	{
	...
	thNode.className = thNode.className.replace(Sortable.REclassName, "") + " "
			+ thNodeClassName;
...
Sortable.createCompare = function(colidx, datatype) {
	return function(row1, row2) {
		// var val1 = Sortable.convert( row1.cells[colidx].firstChild.nodeValue,
		// datatype );
		// var val2 = Sortable.convert( row2.cells[colidx].firstChild.nodeValue,
		// datatype );
...
/**
 * * 200 Collapsable list items * * See also David Lindquist <first name><at><last
 * name><dot><net> * See:
...
Collapsable.ClassNameBoxClosed = "collapseboxClosed"; // same as collapsebox, but initially this box is closed
Collapsable.ClassNameBody = "collapsebody";
Collapsable.CollapseID = "clps"; // prefix for unique IDs of inserted DOM
									// nodes
...
Collapsable.onPageLoad = function(context) {
	Collapsable.OpenTip = i18n.message("ajax.jspwiki.collapsable.opentip");
	Collapsable.CloseTip = i18n.message("ajax.jspwiki.collapsable.closetip");

	this.initialise($("#favorites")[0], this.CookiePrefix + "Favorites");

	var cookieName = this.CookiePrefix + Wiki.getPageName();
	var self = this;
	findWikiContent(context).each(function() {
		self.initialise(this, cookieName);
	});
}

Collapsable.initialise = function(domElem, cookieName) {
	if (!domElem) {
		return;
	}
	this.tmpcookie = jspwikiExt.getCookie(cookieName);
	this.cookies.push(""); // initialise new empty collapse cookie
	this.cookieNames.push(cookieName);

	var nodes;
	nodes = getElementsByClassName(domElem, this.ClassName);
	if (nodes) {
		for ( var i = 0; i < nodes.length; i++)
			this.collapseNode(nodes[i]);
	}
	nodes = getElementsByClassName(domElem, this.ClassNameBox);
	if (nodes) {
		for ( var i = 0; i < nodes.length; i++)
			this.collapseBox(nodes[i], false);
	}
	nodes = getElementsByClassName(domElem, this.ClassNameBoxClosed);
	if (nodes) {
		for ( var i = 0; i < nodes.length; i++)
			this.collapseBox(nodes[i], true);
	}
}

Collapsable.REboxtitle = new RegExp("h2|h3|h4");
Collapsable.collapseBox = function(node, closed) {
	if ($(node).attr('collapsable_initialized')) {
		return; // avoid double initialization
	}
	$(node).attr('collapsable_initialized', true);

	var title = node.firstChild;
	while ((title != null)
			&& (!this.REboxtitle.test(title.nodeName.toLowerCase()))) {
	...
	this.initBullet(bullet, body, (closed ? this.MarkerClose : this.MarkerOpen));
	title.appendChild(bullet);
}

// Modifies the list such that sublists canbe hidden and shown by clicking the
// listitem bullet
// The listitem bullet is a node inserted into the DOM tree as the first child
// of the
...
Collapsable.collapseNode = function(node) {
	...
	for (i = 0; i < items.length; i++) {
		var nodeLI = items[i];
		var nodeXL = (nodeLI.getElementsByTagName("ul")[0] || nodeLI
				.getElementsByTagName("ol")[0]);

		// dont insert bullet when LI is "empty" -- iow it has no text or no non
		// ulol tags inside
		// eg. * a listitem
		...
		for ( var n = nodeLI.firstChild; n; n = n.nextSibling) {
			...
			if ((n.nodeName == "UL") || (n.nodeName == "OL"))
				break; // seems like an empty li
			emptyLI = false;
			break;
		}
		if (emptyLI)
			continue; // do not insert a bullet

		var bullet = this.bullet.cloneNode(true);

		if (nodeXL) {
			var defaultState = (nodeXL.nodeName == "UL") ? this.MarkerOpen
					: this.MarkerClose;
...
Collapsable.initBullet = function(bullet, body, defaultState) {
	...
	var stateOpen = collapseState == this.MarkerOpen;
	this.setOpenOrClose(bullet, stateOpen, body);

	if (!(stateOpen) && location.hash)
	{
		// reset the scroll position with the build-in
		// scroll positioning using a short timeout
		setTimeout( function(){
...
Collapsable.parseCookie = function(token) {
	...
	if ((this.tmpcookie) && (this.tmpcookie.length > currentcookie.length)) {
		...
		if (((token == this.MarkerOpen) && (cookieToken == this.MarkerClose))
			...
			token = cookieToken;
		if (token != cookieToken) // mismatch between tmpcookie and expected
									// token
			this.tmpcookie = null;
	}
	this.cookies[this.cookies.length - 1] += token; // append and save
													// currentcookie
...
Collapsable.toggleBullet = function() {
	...
	if (ctx.REboxtitle.test(this.parentNode.nodeName.toLowerCase())) {
		...
		body = (this.parentNode.getElementsByTagName("ul")[0] || this.parentNode
				.getElementsByTagName("ol")[0]);
	}
	if (!body)
		return;
	...
	var c = (cookie.charAt(i) == ctx.MarkerOpen) ? ctx.MarkerClose
			: ctx.MarkerOpen;
	cookie = cookie.substring(0, i) + c + cookie.substring(i + 1);

	jspwikiExt.setCookie(ctx.cookieNames[idARR[1]], cookie);
...
/**
 * * 130 GraphBar Object : also used on the findpage * %%graphBars ... %% *
...
GraphBar.onPageLoad = function() {
	var g = jspwikiExt.getElementsByClassName(this.REclassName);
	if (!g)
		return;
	...
	for ( var i = 0; i < g.length; i++) {
		...
		var parms = RegExp.$1.split('-');
		var color = (parms[1] ? "style='background:" + parms[1].parseColor()
				+ ";color:" + parms[1].parseColor() + ";' " : "");
		var maxsize = (parms[2] ? parseInt(parms[2], 10) : 120);

		var gBars = getElementsByClassName(g[i], "gBar");
		if (!gBars)
			continue;

		var gBarD = [], maxValue = Number.MIN_VALUE;
		...
		for ( var j = 0; j < gBars.length; j++) {
			var s = ".".repeat(parseInt(maxsize * (gBarD[j] - minValue)
					/ maxValue) + 1);
			gBars[j].innerHTML = " <span class='graphBar' " + color + ">" + s
					+ "</span> " + gBarD[j];
		}
	}
}
