$(document).ready(function() {
	...
	function insertToEditor(wikiLinkHtml) {
		console.log("inserting html to froala:" + wikiLinkHtml);
		editor.$oel.froalaEditor("html.insert", wikiLinkHtml, true);
		...
		if ($("#searchTab-tab").hasClass("ditch-focused")) {
			...
			if (selectedRadioButtons.length > 0) {
				...
				if ($("#search-issues-tab").hasClass("ditch-focused")) {
					...
					insertToEditor(link[0].outerHTML);
				} else {
					var rev = "";
					var href= "";
					var alt = "";
			...
			try {
				...
				if (selectedRadioButtons.length > 0) {
					siblingLink = selectedRadioButtons.closest('tr').find('a');
					wikiLinkHtml = createWikiLinkHtmlFromElement(siblingLink[1]);
