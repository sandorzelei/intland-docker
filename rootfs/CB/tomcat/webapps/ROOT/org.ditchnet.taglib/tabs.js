...
org.ditchnet.jsp.TabUtils.switchTab = function (eventSource) {
	with (org.ditchnet.jsp.TabUtils) {
		...
		var selectedTabPane = org.ditchnet.jsp.TabUtils.tabPanes[selectedIndex];
		var tabId = null;
		if (selectedTabPane != null) {
			tabId = selectedTabPane.id;
		}
		setTabCookie(tabId);
	}
};
...
/**
 *	@param number selectedTabId The id of the selected tab (not the index!)
 *	@returns void
 */
org.ditchnet.jsp.TabUtils.setTabCookie = function (selectedTabId) {
	with (org.ditchnet) {
		var name  = "org.ditchnet.jsp.tabs."+jsp.TabUtils.tabContainer.id;
		var c = new util.Cookie(name,selectedTabId);
...
org.ditchnet.event.Event.prototype.consume = function () {
	// this._evt.currentTarget supported by FF
	// this._evt.srcElement supported by IE
	if (this._evt.currentTarget) {
