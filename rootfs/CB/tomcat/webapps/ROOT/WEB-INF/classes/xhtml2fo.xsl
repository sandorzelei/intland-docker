...
<xsl:stylesheet version="1.0"
	...
	<!-- CodeBeamer specific -->
	<xsl:param name="hyperlink-baseurl">http://www.codebeamer.com</xsl:param> <!-- used when exporting relative URLs -->
	<xsl:param name="page-header-text"></xsl:param>
	<xsl:param name="page-footer-text"></xsl:param>
	<xsl:param name="wm"></xsl:param>
	...
	<xsl:attribute-set name="h1">
		...
		<xsl:attribute name="border-bottom">0.25px solid #CCC</xsl:attribute>
	</xsl:attribute-set>
	...
	<xsl:attribute-set name="pre">
		<xsl:attribute name="border">thin dotted #3C78B5</xsl:attribute>
		<xsl:attribute name="background-color">#F0F0F0</xsl:attribute>
		<xsl:attribute name="padding">2px</xsl:attribute>
		<xsl:attribute name="margin">2em</xsl:attribute>
	...
	<xsl:attribute-set name="hr">
		<xsl:attribute name="border">0.5px solid #AAA</xsl:attribute>
	...
	<xsl:template match="html:body">
		<fo:bookmark-tree>
			<xsl:apply-templates select="html:h2 | html:h3 | html:h4" mode="bookmarking" />
		</fo:bookmark-tree>
		...
		<fo:page-sequence master-reference="all-pages">
			...
			<fo:static-content flow-name="page-header">
				...
				<fo:block space-before.conditionality="retain"
									xsl:use-attribute-sets="page-header">
					<xsl:if test="$page-header-text">
						<xsl:value-of select="$page-header-text"/>
					</xsl:if>
				</fo:block>
			</fo:static-content>
			<fo:static-content flow-name="page-footer">
				<fo:block space-after.conditionality="retain"
					...
					<xsl:if test="$page-footer-text">
						<xsl:value-of select="$page-footer-text"/>
					</xsl:if>
				</fo:block>
				<fo:block space-after.conditionality="retain"
	...
	<xsl:template name="process-common-attributes">
		...
		<!-- generate unique ID, but do not rely on the input ID as it is not guaranteed that these are unique in the wiki output -->
		...
		<!-- apply specific styling based on the class (for consistency with the web CSS) -->
		<xsl:if test="@class">
			<xsl:choose>
				<xsl:when test="@class = 'wiki-error'">
					<xsl:attribute name="background-color">#FFDDDD</xsl:attribute>
				</xsl:when>
				<xsl:when test="@class = 'information'">
					<xsl:attribute name="background-color">#D8E4F1</xsl:attribute>
					<xsl:attribute name="border">0.5px solid #3C78B5</xsl:attribute>
					<xsl:attribute name="padding">1em</xsl:attribute>
					<xsl:attribute name="margin">0 0 1em 0</xsl:attribute>
				</xsl:when>
				<xsl:when test="@class = 'warning'">
					<xsl:attribute name="background-color">#FFFFCE</xsl:attribute>
					<xsl:attribute name="border">0.5px solid #F0C000</xsl:attribute>
					<xsl:attribute name="padding">1em</xsl:attribute>
					<xsl:attribute name="margin">0 0 1em 0</xsl:attribute>
				</xsl:when>
				<xsl:when test="@class = 'error'">
					<xsl:attribute name="background-color">#FFCCCC</xsl:attribute>
					<xsl:attribute name="border">0.5px solid #CC0000</xsl:attribute>
					<xsl:attribute name="padding">1em</xsl:attribute>
					<xsl:attribute name="margin">0 0 1em 0</xsl:attribute>
				</xsl:when>
				<xsl:when test="@class = 'commentbox'">
					<xsl:attribute name="background-color">#F5F5F5</xsl:attribute>
					<xsl:attribute name="border">0.5px solid #E5E5E5</xsl:attribute>
					<xsl:attribute name="padding">1em</xsl:attribute>
					<xsl:attribute name="margin-left">0</xsl:attribute><!-- removes this redundant attribute -->
					<xsl:attribute name="margin">0 0 1em 0</xsl:attribute>
				</xsl:when>
				<xsl:when test="@class = 'toc'">
					<xsl:attribute name="background-color">#EDF3FE</xsl:attribute>
					<xsl:attribute name="padding">0.75em</xsl:attribute>
				</xsl:when>
				<xsl:when test="@class = 'toclevel-xxl'">
					<xsl:attribute name="background-color">#EDF3FE</xsl:attribute>
				...
				<xsl:when test="@class = 'toclevel-xl'">
					<xsl:attribute name="background-color">#EDF3FE</xsl:attribute>
				...
				<xsl:when test="@class = 'toclevel-1'">
					<xsl:attribute name="start-indent">inherited-property-value(start-indent) + 4em</xsl:attribute>
				</xsl:when>
				<xsl:when test="@class = 'toclevel-2'">
					<xsl:attribute name="start-indent">inherited-property-value(start-indent) + 6em</xsl:attribute>
					<xsl:attribute name="font-size">90%</xsl:attribute>
				</xsl:when>
				<xsl:when test="@class = 'toclevel-3'">
					<xsl:attribute name="start-indent">inherited-property-value(start-indent) + 8em</xsl:attribute>
	...
	<xsl:template name="process-style">
		...
		<xsl:if test="$name">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<!-- ignore all unknown styling, because they cause FO syntax problems and the PDF export will fail -->
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
	...
	<xsl:template match="html:h2 | html:h3 | html:h4" mode="bookmarking">
	...
	<xsl:template match="html:p">
		<xsl:if test="string(.) or count(child::*)"> <!-- skip empty paragraphs generated by the wiki renderer -->
	...
	<xsl:template match="html:th">
		<xsl:if test="not(@class) or (@class and @class != 'colseparator')"><!-- do not format headers of separator columns -->
	...
	<xsl:template match="html:td">
		<xsl:if test="not(@class) or (@class and @class != 'colseparator')"><!-- do not format cells of separator columns -->
	...
	<xsl:template match="html:span">
		<xsl:choose>
			<xsl:when test="@class = 'information' or @class = 'warning' or @class = 'error' or @class='commentbox'">
				<fo:block>
					<xsl:call-template name="process-common-attributes-and-children"/>
				</fo:block>
			</xsl:when>
	...
	<xsl:template name="process-img">
		...
		<xsl:if test="@width">
			<xsl:choose>
				...
				<xsl:otherwise>
					<!-- ignore size specified in pixels -->
				</xsl:otherwise>
			</xsl:choose>
		</xsl:if>
		<xsl:if test="@height">
	...
	<xsl:template name="process-a-link">
		<xsl:call-template name="process-common-attributes"/>
		<xsl:choose>
			<xsl:when test="starts-with(@href,'#') and string-length(@href) > 1">
			...
			<xsl:when test="@href">
				<xsl:attribute name="external-destination">
					...
					<xsl:if test="starts-with(@href,'/')">
						<xsl:value-of select="$hyperlink-baseurl"/>
					</xsl:if>
					<xsl:value-of select="@href"/>
					<xsl:text>')</xsl:text>
				</xsl:attribute>
			</xsl:when>
			<xsl:otherwise> <!-- fall back to base URL for FO if no URL was available -->
				<xsl:attribute name="external-destination">
					<xsl:value-of select="$hyperlink-baseurl"/>
				</xsl:attribute>
			</xsl:otherwise>
