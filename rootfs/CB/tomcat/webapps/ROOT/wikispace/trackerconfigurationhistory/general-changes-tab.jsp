...
<c:if test="${fn:length(mapEntry.value.trackerChanges) == 0}">
...
<c:if test="${fn:length(mapEntry.value.trackerChanges) > 0}">
    ...
    <table class="tracker-history-entries"${tableStyle}>
        ...
        <c:forEach var="auditEntry" items="${mapEntry.value.trackerChanges}">
            <c:if test="${not auditEntry.baselineEntry}">
                <c:forEach var="change" items="${auditEntry.configurationChanges}" varStatus="loop">
                    <tr>
                        ...
                        <td><spring:message code="tracker.configuration.history.${change.action}.label" /></td>
            ...
            <jsp:include page="audit-baseline-row.jsp">
                <jsp:param value="4" name="columnCount" />
            </jsp:include>
        </c:forEach>
    </table>
</c:if>
