	...
	<ui:actionMenuBar>
		<jsp:body>
			<ui:breadcrumbs showProjects="false" projectAware="${copy}"><span class='breadcrumbs-separator'>&raquo;</span>
				<c:set var="originalName" value="" scope="request"/>
				<c:if test="${original != null}">
					<c:set var="originalName" value="${original.name}" scope="request"/>
				</c:if>
				<span><spring:message code="diffTool.pageTitle" text="Differences" arguments="${copy.name},${originalName}" htmlEscape="true" /></span>
				<ui:pageTitle prefixWithIdentifiableName="false" printBody="false" >
					<spring:message code="diffTool.pageTitle" text="Differences" arguments="${copy.name},${originalName}" htmlEscape="true" />
				</ui:pageTitle>
			</ui:breadcrumbs>
		</jsp:body>
	</ui:actionMenuBar>
...
<c:choose>
	...
	<c:otherwise>
		...
		<jsp:include page="artifactDiffTable.jsp"/>
	</c:otherwise>
</c:choose>
